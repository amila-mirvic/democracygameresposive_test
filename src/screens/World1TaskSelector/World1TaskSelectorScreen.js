...
export default function World1TaskSelectorScreen() {
  ...
  const computeTotalsAndBadges = () => {
    ...
    if (t1) {
      if (typeof t1.points === "number") totalPoints += t1.points;
      if (typeof t1.curiosityPoints === "number")
        totalCuriosityPoints += t1.curiosityPoints;
      if (Array.isArray(t1.badges)) badges.push(...t1.badges);
    }
  ...
  /* ✅ TASK HANDLERS */
  const handleTask1 = () => {
    // ✅ vodi na Task 1 screen (ruta mora postojati u App.js)
